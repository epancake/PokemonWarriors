    ...
    this.fetchTypesService.getJSON().subscribe((data: any) => {
      data.map(item => {
        if (type.type.name === item.type) {
          this.battleBias.superEffectiveAgainst = item.superEffectiveAgainst
            .map(entry => entry.charAt(0).toUpperCase() + entry.slice(1));
          this.battleBias.notVeryEffectiveAgainst = item.notVeryEffectiveAgainst
            .map(entry => entry.charAt(0).toUpperCase() + entry.slice(1));
          this.battleBias.resistantTo = item.resistantTo
            .map(entry => entry.charAt(0).toUpperCase() + entry.slice(1));
          this.battleBias.vulnerableTo = item.vulnerableTo
            .map(entry => entry.charAt(0).toUpperCase() + entry.slice(1));
        }
      });
    });
